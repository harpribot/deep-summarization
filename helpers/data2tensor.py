...
class Mapper:
    ...
    def get_tensor(self, reverseFlag=False):
        ...
        if reverseFlag:
            self.review_tensor_reverse = self.__generate_tensor(isReview=True, reverse=True)
    ...
    def __generate_tensor(self,isReview, reverse=False):
        ...
        for index, entry in enumerate(sample.tolist()):
            ...
            if reverse:
                index_lst = index_lst[::-1] # Fastest way to reverse
            # Pad the list
