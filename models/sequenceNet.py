...
class NeuralNet:
    ...
    def generate_one_summary(self,review_fwd, review_bwd):
        ...
        feed_dict_review_fwd.update({self.labels[t]: review_fwd[t] for t in range(self.seq_length)})

        feed_dict_review_bwd = {self.enc_inp_bwd[t]: review_bwd[t] for t in range(self.seq_length)}
        feed_dict_review_bwd.update({self.labels[t]: review_bwd[t] for t in range(self.seq_length)})
